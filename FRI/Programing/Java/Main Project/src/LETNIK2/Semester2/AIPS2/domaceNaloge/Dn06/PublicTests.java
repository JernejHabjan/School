...
public class PublicTests extends TestCase {
	...
	private void fillGraphDirected(ShortestPath g, int npoints, int ncrosslinks, Random r){
		for (int i = 0; i < npoints; i++){
			assertTrue(g.addNode(allTowns.get(i)));
		}
		Vector<String> t = g.getNodes();
	...
	private void fillGraphUndirected_NEGATIVE(ShortestPath g, int npoints, int ncrosslinks, Random r){
		for (int i = 0; i < npoints; i++){
			assertTrue(g.addNode(allTowns.get(i)));
		}
		Vector<String> t = g.getNodes();
...
*/
	...
	public void testDistToEachTownDirected() throws Exception {
		ShortestPath g = new ShortestPath();
		r.setSeed(35);
		fillGraphDirected(g, 5, 10, r);
		final int[][] dists = {
				{0,  8,  6,  8, 13},
				{8,  0, 14, 16,  2},
				{6, 14,  0,  8,  9},
		...
		for (int i = 1; i < 2; i++){
			...
			for (String key:g.edges.keySet()){
				System.out.println(key);
				for(Pair<String, Integer> l:g.edges.get(key)){
					System.out.println("    "+l.getKey());
				}

			}

			System.out.println("Loooooooooooooooooollllllll");


			for (int j = 0; j < 5-i; j++){

				System.out.println(g.getShortestPath(towns.elementAt(i), towns.elementAt(j)));
				//assertEquals(dists[i][j], g.getShortestDist(towns.elementAt(i), towns.elementAt(j)));
			}
		}
	}
	/*
	public void testDistToEachTownUndirected_TEST_NEGATIVE() throws Exception {
		ShortestPath g = new ShortestPath();
		r.setSeed(35);
		fillGraphUndirected_NEGATIVE(g, 5, 1, r);
		final int[][] dists = {
				{0,  8,  6,  8, 13},
				{8,  0, 14, 16,  2},
				{6, 14,  0,  8,  9},
