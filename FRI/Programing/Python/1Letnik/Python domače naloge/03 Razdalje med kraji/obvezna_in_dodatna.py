...
for ime, x, y in kraji:
    razdalja = sqrt((x - location1x) ** 2 + (y - location1y) ** 2)
    location1x = x
    location1y = y
    skupnarazdalja += razdalja

    diagonala_razdalje = sqrt((koord_x - x) ** 2 + (koord_y - y) ** 2)
    if max_razdalja < diagonala_razdalje:
        max_razdalja = diagonala_razdalje
        max_kraj = ime

print("maksimalno oddaljen kraj je : ", max_kraj)
print("razdalja med zaporednimi kraji je: ", skupnarazdalja)
# obvezna in dodatna naloga
