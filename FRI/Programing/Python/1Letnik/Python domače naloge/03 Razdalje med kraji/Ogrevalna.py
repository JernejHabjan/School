...
"""for elementi in zip(kraji):
    for plac,x,y in elementi:
        print(plac)"""
# Uporabnik vnese koordinato x in koordinato y. Program izpiše imena vseh krajev in njihove razdalje od teh koordinat.
koord_x = float(input("vpisi koordinato x: "))
koord_y = float(input("vpisi koordinato y: "))
"""for elementi in zip(kraji):
    for plac,x,y in elementi:
        print(plac)
        print("razdalja po osi x je: ",abs(koord_x-x))
        print("razdalja po osi y je: ",abs(koord_y-y))"""
# Uporabnik vnese koordinati, tako kot prej, program pa izpiše le razdaljo do najbolj oddaljenega kraja.
max_razdalja = 0
for elementi in zip(kraji):
    for plac, x, y in elementi:
        razdalja_x = abs(koord_x - x)
        razdalja_y = abs(koord_y - y)
        diagonala_razdalje = sqrt((razdalja_x) ** 2 + (razdalja_y) ** 2)
        if max_razdalja < diagonala_razdalje:
            max_razdalja = diagonala_razdalje

print("maksimalna razdalja je: ", max_razdalja)
