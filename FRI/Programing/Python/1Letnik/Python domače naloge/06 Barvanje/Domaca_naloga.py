...
def prestej_barvo(sahovnica, barva):
    pogostosti = 0
    for polje, barv in sahovnica.items():
        if barv == barva:
            pogostosti += 1
    return pogostosti
...
def polja_po_barvi(sahovnica, barva):
    koordinate = set()
    for koord, barv in sahovnica.items():
        if barva == barv:
...
def prestej_barve(sahovnica):
    ...
    for koord, barva in sahovnica.items():
        pogostosti[barva] += 1
    return pogostosti
...
def polja_po_barvah(sahovnica):
    ...
    for koord, barva in sahovnica.items():
        mnozica = set()
        for kord1, barv in sahovnica.items():
            if barv == barva:
                mnozica.add(kord1)
        slovar[barva] = mnozica
    return slovar
