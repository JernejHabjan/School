# http://localhost:63342/Python/Python%20doma%C4%8De%20naloge/10%20Kalkulator/calculator/html/index.html
# https://ucilnica.fri.uni-lj.si/mod/assign/view.php?id=10328
# Ocena 6 / Grade 6
...
def parse(s):
    ...
    for element in s.split("\n"):
        ele = element.split()
        if len(ele) == 5:
            return (to_number(ele[-1]), ele[1], (to_number(ele[0]), to_number(ele[2])))
        elif len(ele) == 4:
            return (to_number(ele[3]), ele[0], (to_number(ele[1]),))
        elif len(ele) == 3:
            return (to_number(ele[2]), "SET", (to_number(ele[0]),))
...
def read(filename):
    ...
    return [parse(element) for element in open(filename)]
...
def inputs(exprs):
    ...
    return {ele for element in exprs for ele in element[2] if isinstance(ele, str)}
...
def outputs(exprs):
    ...
    return set(ele for element in exprs for ele in element[0] if isinstance(ele, str))
...
def check_names(exprs):
    ...
    """
    for x in inputs(exprs):
        if x not in outputs(exprs):
            return False
    return True
...
def check_operators(exprs):
    ...
    for element in exprs:
        if element[1] not in valid_operators:
            return False
    return True
...
def get_values(args, variables):
    ...
    """my_list=[]
    for element in args:
        my_list.append(get_value(element, variables))
    return tuple(my_list)"""
    return tuple(get_value(element, variables) for element in args)
...
def compute_expr(op, args):
    ...
    if op == "LSHIFT": return (args[0] << args[1])
    if op == "RSHIFT": return (args[0] >> args[1])
...
def compute_list(exprs):
    ...
    for element in exprs:
        result=element[0]
        op=element[1]
        args=get_values(element[2],my_dict)
        my_dict[result]=compute_expr(op, args)
    return my_dict"""
    my_dict = dict()
    for element in exprs:
        my_dict[element[0]] = compute_expr(element[1], get_values(element[2], my_dict))
    return my_dict
...
def compute(var, exprs, variables):
    ...
    for arg in args:
        if isinstance(arg, str) and arg not in variables:
            variables[arg] = compute(arg, exprs,
                                     variables)  # to kar sem enkat izracunal si zapovnem - variables... to je vse kar jaz ze vem
...
"""
    """if not check_names(exprs):
        return False
    variables={}
    neki =dict_expr(exprs)
    try:
        for var, op, args in exprs:
            compute(var,neki,variables)
    except:
        return False
    return True"""
