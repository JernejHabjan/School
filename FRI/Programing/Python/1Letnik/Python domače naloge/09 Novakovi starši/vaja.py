def starost_starsa(oseba):
    ...
    for posameznik in rodbina[oseba]:
        maksimum.append(starost[posameznik])
    if len(maksimum) == 0:
        return 999
    else:
        return int(starost[oseba] - max(maksimum))
...
def starsi_pred(oseba, n):
    ...
    for posameznik in otroci(oseba):
        mnozica = mnozica.union(starsi_pred(posameznik, n))
    print(mnozica)
    return mnozica
...
"""def starsi_pred(oseba, n):
    mnozica=set()
    trenutna_starost=starost_starsa(oseba)
    for posameznik in otroci(oseba):
        trenutna_starost=starost_starsa(posameznik)
        if trenutna_starost<n:
            mnozica.add(starsi_pred(posameznik, n ))
    return mnozica"""
