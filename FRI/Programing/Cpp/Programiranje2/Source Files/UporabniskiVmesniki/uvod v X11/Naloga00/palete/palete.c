...
/* Inicializacija */
    ...
    if (BEST == 1)
        system("/usr/bin/xstdcmap -best");
    else
        system("/usr/bin/xstdcmap -default");
...
/* Bela barva - barva ozadja */
    ...
    for (j = 0; j < 256; j++) {
        for (k = 0; k < 256; k++) {
            pixel = stdcolor.base_pixel + (j % 256 * stdcolor.red_mult) + (k % 256 * stdcolor.green_mult) +
                    (l % 256 * stdcolor.blue_mult);
            status = XPutPixel(image, j, k, pixel);
        }
    }
