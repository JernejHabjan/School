...
FreeTypeFont::FreeTypeFont() { bLoaded = false; }
...
bool FreeTypeFont::loadSystemFont(string sName, int iPXSize)
{
	...
	std::string sPath = "C:\\Windows"; ////////////////////////// TODO HARDCODED
	sPath += "\\Fonts\\";
	sPath += sName;
...
void FreeTypeFont::printFont(std::string sText, int x, int y, int iPXSize)
{

	if (!bLoaded){
		print("font not loaded... exiting...");
		system("PAUSE");
		//exit(1);
		return;
	}
