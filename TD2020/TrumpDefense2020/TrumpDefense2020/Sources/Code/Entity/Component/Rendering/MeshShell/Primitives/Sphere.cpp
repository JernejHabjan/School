...
void Sphere::init(APIuint vertexPositionID)
{
    int i, j;
    std::vector<float> vertices;
    std::vector<APIuint> indices;
    int indicator = 0;
    for(i = 0; i <= lats; i++) {
       ...
       //indices.push_back(GL_PRIMITIVE_RESTART_FIXED_INDEX); //TODO
   }
    ...
    //glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(APIuint), &indices[0], GL_STATIC_DRAW);
